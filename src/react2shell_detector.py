...
class CloudTrailAnalyzer:
    ...
    def _analyze_event(
        self, event: Dict, event_type: str, malicious_ips: Set[str]
    ) -> Optional[Finding]:
        ...
        try:
            ...
            if user_identity.get(
                "type"
            ) == "AssumedRole" and "EC2" in user_identity.get("arn", ""):
                ...
                if event_type in [
                    ...
                    return Finding(
                        ...
                        resource_id=user_identity.get("principalId", "unknown").split(
                            ":"
                        )[-1],
                        ioc_type="BEHAVIOR",
...
class VPCFlowLogAnalyzer:
    ...
    def analyze_flow_logs(self, log_group: str, hours: int = 24) -> List[Finding]:
...
class GuardDutyManager:
    ...
    def create_threat_intel_set(
        self, bucket_name: str, detector_id: Optional[str] = None
    ) -> Optional[str]:
        ...
        try:
            ...
            for set_id in existing_sets.get("ThreatIntelSetIds", []):
                set_info = self.guardduty.get_threat_intel_set(
                    DetectorId=detector_id, ThreatIntelSetId=set_id
                )
    ...
    def get_relevant_findings(self, hours: int = 24) -> List[Finding]:
        ...
        try:
            ...
            for page in paginator.paginate(
                ...
                FindingCriteria={
                    "Criterion": {
                        "type": {"Eq": relevant_finding_types},
                        "updatedAt": {
                            "Gte": int(
    ...
    def _convert_guardduty_finding(self, gd_finding: Dict) -> Finding:
        ...
        return Finding(
            ...
            timestamp=datetime.fromisoformat(
                gd_finding.get("UpdatedAt", datetime.utcnow().isoformat()).replace(
                    "Z", ""
                )
            ),
...
class WAFLogAnalyzer:
    ...
    def analyze_waf_logs(self, log_group: str, hours: int = 24) -> List[Finding]:
    ...
    def _analyze_waf_request(self, request: Dict) -> Optional[Finding]:
        ...
        try:
            headers = (
                json.loads(headers_str) if isinstance(headers_str, str) else headers_str
            )
        except json.JSONDecodeError:
            headers = {}
...
class OrganizationScanner:
    ...
    def assume_role_in_account(
        self, account_id: str, role_name: str = "OrganizationAccountAccessRole"
        ...
        with ThreadPoolExecutor(max_workers=max_workers) as executor:
            ...
            for account in accounts:
                future = executor.submit(
                    self._scan_account, account["id"], role_name, hours
                )
    ...
    def _scan_account(
        self, account_id: str, role_name: str, hours: int
    ) -> List[Finding]:
    ...
    def import_findings(self, findings: List[Finding]) -> Tuple[int, int]:
        ...
        for i in range(0, len(findings), batch_size):
            ...
            try:
                ...
                response = self.securityhub.batch_import_findings(Findings=sh_findings)
...
def main():
    """Main entry point"""
    parser = argparse.ArgumentParser(description="React2Shell AWS IOC Detection Script")
    parser.add_argument(
        "--config", default="config/iocs.yaml", help="Path to IOC configuration file"
    )
    parser.add_argument(
        "--hours", type=int, default=24, help="Hours of logs to analyze (default: 24)"
    )
    parser.add_argument(
        "--organization",
        action="store_true",
    ...
    parser.add_argument(
        "--security-hub", action="store_true", help="Import findings to Security Hub"
    )
    parser.add_argument(
        "--guardduty-bucket", help="S3 bucket for GuardDuty threat intel"
    )
    parser.add_argument("--vpc-log-group", help="VPC Flow Logs CloudWatch log group")
    parser.add_argument("--waf-log-group", help="WAF logs CloudWatch log group")
    parser.add_argument(
        "--output",
    ...
    if args.organization:
        ...
        all_findings.extend(
            org_scanner.scan_organization(hours=args.hours, role_name=args.role_name)
        )
    else:
        # Single account scan
        ...
        if args.vpc_log_group:
            ...
            all_findings.extend(
                vpc_analyzer.analyze_flow_logs(args.vpc_log_group, args.hours)
            )
        ...
        if args.waf_log_group:
            ...
            all_findings.extend(
                waf_analyzer.analyze_waf_logs(args.waf_log_group, args.hours)
            )
